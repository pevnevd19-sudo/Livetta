...
async function init() {
  try {
    ...
    renderStonesCatalog();
    updateSummary();
    window.__livettaConstructorModuleReady = true;

    loadStonesCatalog()
      .then((stones) => {
        stonesCatalog = Array.isArray(stones) ? stones : [];
        renderStonesCatalog();
        updateSummary();
      })
      .catch((error) => {
    ...
    console.error(error);
    window.__livettaConstructorModuleReady = false;
...
async function loadStonesCatalog() {
  try {
    const response = await fetchWithTimeout(`${API_URL}/stones?cache=${Date.now()}`, 5000);
    ...
    if (!stones.length) {
      capacityHint.textContent = 'Камни пока не добавлены. Скоро здесь появятся новые варианты для сборки.';
    } else {
      capacityHint.textContent = 'Выберите камни в каталоге и соберите свое колье.';
    }

    return stones;
  } catch (error) {
    console.warn('Камни не загрузились:', error.message);
    capacityHint.textContent = 'Камни временно не загрузились. Конструктор уже открыт, попробуйте обновить страницу чуть позже.';
    return [];
  }
}
...
function bindEvents() {
  ...
  openStonesCatalog?.addEventListener('click', openStoneCatalogModal);
  closeStoneCatalog?.addEventListener('click', closeStoneCatalogModal);

  document.addEventListener('click', (event) => {
    if (event.target.closest('[data-close-stones-catalog]')) {
      closeStoneCatalogModal();
    }
  });

  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && stoneCatalogModal && !stoneCatalogModal.hidden) {
      closeStoneCatalogModal();
    }
  });
  ...
  stonesList.addEventListener('click', (event) => {
    ...
    addStone(stone);
    focusNeckStageOnMobile();
  });

  stoneCatalogList?.addEventListener('click', (event) => {
    const button = event.target.closest('[data-catalog-add-stone-id]');

    if (!button || button.disabled) {
      return;
    }

    const stone = stonesCatalog.find((item) => item.id === button.dataset.catalogAddStoneId);
    addStone(stone);
    renderStoneCatalog();
    focusNeckStageOnMobile();
  });
...
function setupResponsiveStonesPanel() {
  return;
}

function focusNeckStageOnMobile() {
  return;
}

function renderStonesCatalog() {
  if (!stonesCatalog.length) {
    stonesList.innerHTML = '<p class="muted-text">Камней пока нет. Скоро здесь появятся новые варианты.</p>';
    renderStoneCatalog();
    return;
  }
    ...
    return `
      <button class="stone-card constructor-stone-card ${disabled ? 'stone-card--disabled' : ''}" type="button" data-stone-id="${escapeHtml(stone.id)}" ${disabled ? 'disabled' : ''}>
        ...
        <span class="stone-card__content">
          ...
          ${stone.zodiac ? `<small class="stone-zodiac">Зодиак: ${escapeHtml(stone.zodiac)}</small>` : ''}
...
function renderStoneCatalog() {
  if (!stoneCatalogList) return;

  if (!stonesCatalog.length) {
    stoneCatalogList.innerHTML = '<p class="muted-text">Камней пока нет. Скоро здесь появятся новые варианты.</p>';
    return;
  }

  stoneCatalogList.innerHTML = stonesCatalog.map((stone) => {
    const canAdd = getCanAddCount(stone);
    const disabled = stone.available === false || canAdd <= 0;
    const image = stone.image ? `<img src="${escapeHtml(stone.image)}" alt="${escapeHtml(stone.name)}" loading="lazy">` : '';

    return `
      <article class="constructor-catalog-stone ${disabled ? 'is-disabled' : ''}">
        <div class="constructor-catalog-stone__image" style="--stone-color:${escapeHtml(stone.color)}">
          ${image || '<span class="stone-card__gem"></span>'}
        </div>
        <div class="constructor-catalog-stone__body">
          <div class="constructor-catalog-stone__meta">
            <span>${formatPrice(stone.price)} ₽</span>
            <span>${formatNumber(stone.sizeMm)} мм</span>
            ${stone.zodiac ? `<span>Зодиак: ${escapeHtml(stone.zodiac)}</span>` : ''}
          </div>
          <h3>${escapeHtml(stone.name)}</h3>
          <p>${escapeHtml(stone.description)}</p>
          ${stone.stoneProperty ? `<div class="constructor-catalog-stone__property"><b>Свойство камня</b><p>${escapeHtml(stone.stoneProperty)}</p></div>` : ''}
        </div>
        <button type="button" data-catalog-add-stone-id="${escapeHtml(stone.id)}" ${disabled ? 'disabled' : ''}>
          ${stone.available === false ? 'Нет в наличии' : (disabled ? 'Не помещается' : 'Добавить в конструктор')}
        </button>
      </article>
    `;
  }).join('');
...
function buildBeadLayout(stones) {
  if (!stones.length || !necklacePath || !neckStage || !necklaceBeads) {
    return [];
  }

  const pathLength = necklacePath.getTotalLength();
  const scale = getSvgPathScale();
  const averageScale = Math.max(scale.average, 0.001);
  ...
  return items.map((item) => {
    ...
    const layerPoint = svgPointToBeadsLayerPoint(point);
    const nextLayerPoint = svgPointToBeadsLayerPoint(nextPoint);
    const angle = Math.atan2(nextLayerPoint.y - layerPoint.y, nextLayerPoint.x - layerPoint.x) * 180 / Math.PI;
...
function getSvgPathScale() {
  const fallbackRect = neckStage?.getBoundingClientRect?.();
  const fallbackScaleX = fallbackRect ? fallbackRect.width / 1000 : 1;
  const fallbackScaleY = fallbackRect ? fallbackRect.height / 1000 : 1;

  try {
    const matrix = necklacePath?.ownerSVGElement?.getScreenCTM?.();
    if (matrix) {
      const scaleX = Math.hypot(matrix.a, matrix.b);
      const scaleY = Math.hypot(matrix.c, matrix.d);

      if (Number.isFinite(scaleX) && Number.isFinite(scaleY) && scaleX > 0 && scaleY > 0) {
        return { x: scaleX, y: scaleY, average: (scaleX + scaleY) / 2 };
      }
    }
  } catch {}
  ...
  return {
    x: fallbackScaleX,
    y: fallbackScaleY,
    average: (fallbackScaleX + fallbackScaleY) / 2
  };
}

function svgPointToBeadsLayerPoint(point) {
  const layerRect = necklaceBeads.getBoundingClientRect();
  const svg = necklacePath?.ownerSVGElement;
  ...
  return {
    x: point.x / 1000 * Math.max(layerRect.width, 1),
    y: point.y / 1000 * Math.max(layerRect.height, 1)
  };
}
...
function pointerToSvgPoint(event) {
  ...
  return {
    x: (event.clientX - stageRect.left) / Math.max(stageRect.width, 1) * 1000,
    y: (event.clientY - stageRect.top) / Math.max(stageRect.height, 1) * 1000
  };
}
...
function updateSummary() {
  ...
  if (selectedClaspMaterial) selectedClaspMaterial.textContent = material ? material.name : 'Не выбран';
  ...
  if (!clasp) {
    ...
    capacityHint.textContent = 'Выберите материал замка — без него сборку нельзя добавить в корзину.';
  } else if (!selectedStones.length) {
    capacityHint.textContent = `Нить свободна. С учётом замка «${clasp.name}» и материала «${material.name}» доступно ${formatNumber(max)} мм под камни.`;
  } else {
...
async function addDesignToCart() {
  ...
  try {
    ...
    const cartClasp = { ...clasp, material: material.name, material_id: material.id };
...
function getClosestPathPositionFromPointer(event) {
  const svgPointer = pointerToSvgPoint(event);
  const pathLength = necklacePath.getTotalLength();
  const samples = 220;
  ...
  for (let i = 0; i <= samples; i += 1) {
    ...
    const dx = point.x - svgPointer.x;
    const dy = point.y - svgPointer.y;
  ...
  const layerPoint = svgPointToBeadsLayerPoint(bestPoint);
...
function getAvailableNecklacePathPx() {
  ...
  return pathLength * Math.max(scale.average, 0.001) * 0.995;
}
...
function getSelectedClaspMaterial() {
  return CLASP_MATERIALS[String(claspMaterial?.value || '')] || null;
}
